#!/usr/bin/env python3
"""
THE CLARITY PROTOCOL — Premium PDF Builder
Uses Chrome headless for high-fidelity PDF rendering with full CSS support.
"""
...
:root {
    ...
    --code-bg: #1a1a1a;
    --code-text: #e8e8e8;
    --cover-gradient-1: #0a0a0a;
    --cover-gradient-2: #1a1a1a;
}
...
def build_html(title, html_body):
    """Wrap HTML body in full document with styling."""
...
<body>
    <div class="cover">
        ...
        <div class="cover-footer">CLARITY PROTOCOL &mdash; CONFIDENTIAL</div>
    </div>
    <div class="content">
        {html_body}
    </div>
...
def build_pdf(title, source_files, output_path):
    """Build a premium PDF from source markdown files."""
    combined = ""
    for src in source_files:
        filepath = os.path.join(SRC, src)
    ...
    if not combined.strip():
        print(f"     X No content for {title}")
        return False
    ...
    try:
        ...
        if chrome_to_pdf(html_path, output_path):
            size_kb = os.path.getsize(output_path) / 1024
            print(f"     OK  {os.path.basename(output_path)} ({size_kb:.0f} KB)")
            return True
        else:
            print(f"     ERR Chrome failed for {title}")
            return False
...
def main():
    ...
    print()
    print("  ╔══════════════════════════════════════════╗")
    print("  ║   THE CLARITY PROTOCOL — PDF BUILDER     ║")
    print("  ║   Premium Edition • Chrome Renderer      ║")
    print("  ╚══════════════════════════════════════════╝")
    print()
    ...
    for folder, pdfs in PDFS.items():
        ...
        for pdf_name, sources in pdfs:
            total += 1
            output_file = os.path.join(folder_path, f"{pdf_name}.pdf")
            if build_pdf(pdf_name, sources, output_file):
                success += 1

        print()

    print(f"  ╔══════════════════════════════════════════╗")
    print(f"  ║   COMPLETE: {success}/{total} Premium PDFs             ║")
    print(f"  ╚══════════════════════════════════════════╝")
